...
<body>
  ...
  $sql = "select distinct(id) from instructor;";
  $result = $con->query($sql);
  $arr = [];
  while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
    array_push($arr, $row);
  ...
  $var = json_encode($arr);
  echo "<script>var instructor_data=$var;</script>";
  ...
  echo"<script>var teach=new Map()</script>";
  foreach($arr as $x){
    echo "<script>
    if(!teach.has('".$x['course_id']."_".$x['sec_id']."_".$x['semester']."'))teach.set('".$x['course_id']."_".$x['sec_id']."_".$x['semester']."',new Set());
    teach.get('".$x['course_id']."_".$x['sec_id']."_".$x['semester']."').add('".$x['id']."');
    </script>";
  }
  ?>
  ...
  <script>
    ...
    function create_table(instructor,course, sec, semester) {
      for (var key in data) {
        //Create each row
        if(instructor&&!teach.get(data[key].course_id+"_"+data[key].sec_id+"_"+data[key].semester).has(instructor))continue;
  ...
  <script>
    ...
    for (var key in instructor_data) {
      var temp_child = (document.createElement('option'));
      temp_child.setAttribute('value',instructor_data[key].id);
      temp_child.setAttribute('selected', false);
      temp_child.appendChild(document.createTextNode(instructor_data[key].id));
      instructor.appendChild(temp_child);
    }
    ...
    instructor.addEventListener('change', () => {
      ...
      var set = new Set();
      for (var key in data) {
        if (teach.get(data[key].course_id+"_"+data[key].sec_id+"_"+data[key].semester).has(instructor.value)) {
          if (set.has(data[key].course_id)) continue;
          else set.add(data[key].course_id);
          var temp_child = (document.createElement('option'));
    ...
    course_id.addEventListener('change', () => {
      ...
      var set = new Set();
      for (var key in data) {
        if (teach.get(data[key].course_id+"_"+data[key].sec_id+"_"+data[key].semester).has(instructor.value)&&data[key].course_id == course_id.options[course_id.selectedIndex].text) {
      ...
      empty(tbody);
      create_table(instructor.value,course_id.value);
    })
    sec_id.addEventListener('change', () => {
      ...
      empty(tbody);
      create_table(instructor.value,course_id.value,sec_id.value);
    })
    ...
    semester.addEventListener('change', () => {
      ...
      empty(tbody);
      create_table(instructor.value,course_id.value, sec_id.value, semester.value);
    })
